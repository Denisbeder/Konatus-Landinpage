...
export default function pageScroll() {
	$(document).ready(function($) {
		...
		function prevSection(event) {
			...
			var animationParams = selectAnimation(
				animationType,
				middleScroll,
				"prev"
			);
			unbindScroll(
				visibleSection.prev(".cd-section"),
				animationParams[3]
			);
			...
			if (sectionOffset >= -windowHeight && sectionOffset <= 0) {
				...
				switch (animationName) {
					...
					case "gallery":
						if (
							...
							translateY = (-sectionOffset * 100) / windowHeight;
							boxShadowBlur =
								400 * (1 + sectionOffset / windowHeight);
						} else if (
							sectionOffset >= -0.9 * windowHeight &&
							...
							translateY = 0;
							boxShadowBlur =
								(-400 * sectionOffset) / windowHeight;
						}
						break;
					case "catch":
						if (
							...
							translateY = 100;
							boxShadowBlur =
								(1 + sectionOffset / windowHeight) * 160;
						} else {
							translateY =
								(-(10 / 7.5) * sectionOffset * 100) /
								windowHeight;
							boxShadowBlur =
								(-160 * sectionOffset) / (3 * windowHeight);
						}
						break;
					case "opacity":
						translateY = 0;
						scale =
							((sectionOffset + 5 * windowHeight) * 0.2) /
							windowHeight;
				...
				switch (animationName) {
					case "scaleDown":
						scale = (
							1 -
							(sectionOffset * 0.3) / windowHeight
						).toFixed(5);
					...
					case "gallery":
						if (
							...
							scale =
								(windowHeight - sectionOffset) / windowHeight;
							...
							translateY =
								(-(9 / 8) *
									(sectionOffset - (0.1 * windowHeight) / 9) *
									100) /
								windowHeight;
							boxShadowBlur = 40;
						} else {
					...
					case "catch":
						if (
							sectionOffset >= 0 &&
							sectionOffset < windowHeight / 2
						) {
							boxShadowBlur = (sectionOffset * 80) / windowHeight;
						} else {
							boxShadowBlur =
								80 * (1 - sectionOffset / windowHeight);
						}
						break;
					case "opacity":
						translateY = 0;
